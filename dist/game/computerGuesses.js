...
import { askGeminiNextMove } from "./geminiGuesser.js";
const MAX_QUESTIONS = 20;
function sleep(ms) {
    return new Promise((resolve) => setTimeout(resolve, ms));
}
export async function runComputerGuesses() {
    ...
    while (count < MAX_QUESTIONS) {
        ...
        count += 1;
        const answer = normalized.startsWith("y") || normalized === "예"
            ? "예"
...
function renderScreen(history, count) {
    console.clear();
...
function formatHistoryLine(entry, index, isCurrent) {
