...
export async function askGeminiNextMove(apiKey, history, wrongGuesses) {
    ...
    try {
        ...
        if (guessMatch)
            return { ok: true, type: "guess", text: guessMatch[1].trim() };
        const questionMatch = text.match(/QUESTION:\s*(.+)/i);
        if (questionMatch)
            return { ok: true, type: "question", text: questionMatch[1].trim() };
